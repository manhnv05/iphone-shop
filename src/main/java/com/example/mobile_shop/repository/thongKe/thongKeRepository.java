...
public interface thongKeRepository extends JpaRepository<HoaDon, Integer> {
    ...
    @Query("SELECT new com.example.mobile_shop.dto.thongKe.SanPhamHetHangDTO(sp.tenSanPham, COUNT(ctsp)) " +
    ...
    // Phân phối đa kênh
    @Query("SELECT new com.example.mobile_shop.dto.thongKe.LoaiHoaDonDTO(hd.loaiDon, COUNT(hd.id)) " +
            "FROM HoaDon hd " +
            "GROUP BY hd.loaiDon")
    List<LoaiHoaDonDTO> thongKeLoaiHoaDon();


    @Query("SELECT new com.example.mobile_shop.dto.thongKe.HangBanChayDTO(nsx.nhaSanXuat, SUM(hdct.gia)) " +

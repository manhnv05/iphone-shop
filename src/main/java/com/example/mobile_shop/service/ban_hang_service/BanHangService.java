package com.example.mobile_shop.service.ban_hang_service;

import com.example.mobile_shop.controller.ban_hang.EmailSendBH;
import com.example.mobile_shop.dto.ban_hang.ChiTietSanPhamGroupDTO;
import com.example.mobile_shop.dto.ban_hang.HDCTban_hangDTO;
import com.example.mobile_shop.dto.ban_hang.HDban_hangDTO;
import com.example.mobile_shop.dto.ban_hang.ThanhToanRequestDTO;
import com.example.mobile_shop.dto.gio_hang.ChiTietGioHangDTO;
import com.example.mobile_shop.dto.gio_hang.GioHangDTO;
import com.example.mobile_shop.entity.*;
import com.example.mobile_shop.entity.SanPham.ChiTietSanPham;
import com.example.mobile_shop.entity.SanPham.ImelDaBan;
import com.example.mobile_shop.repository.giam_gia.PhieuGiamGiaCaNhanRepository;
import com.example.mobile_shop.repository.giam_gia.PhieuGiamGiaRepository;
import com.example.mobile_shop.repository.gio_hang.ChiTietGioHangRepository;
import com.example.mobile_shop.repository.gio_hang.GioHangRepository;
...
import com.example.mobile_shop.repository.hoa_don.HoaDonRepository;
import com.example.mobile_shop.repository.khach_hang.KhachHangRepository;
import com.example.mobile_shop.repository.phuong_thuc_thanh_toan.PhuongThucThanhToanRepository;
import com.example.mobile_shop.repository.san_pham.ChiTietSanPhamRepository;
import com.example.mobile_shop.repository.san_pham.SanPhamRepository;
...
//    }
    //Done
    @Transactional
    public void deleteChiTietGioHang(Integer chiTietGioHangId, Integer gioHangId) {
        System.out.println("Attempting to delete ChiTietGioHang with ID: " + chiTietGioHangId + ", GioHangId: " + gioHangId);
        ChiTietGioHang chiTietGioHang = chiTietGioHangRepository.findById(chiTietGioHangId)
                .orElseThrow(() -> new IllegalArgumentException("Không tìm thấy chi tiết giỏ hàng với ID: " + chiTietGioHangId));
        if (!chiTietGioHang.getIdGioHang().getId().equals(gioHangId)) {
            throw new IllegalArgumentException("Chi tiết giỏ hàng không thuộc giỏ hàng được chỉ định.");
        }
        if (chiTietGioHang.getDeleted()) {
            throw new IllegalArgumentException("Chi tiết giỏ hàng với ID: " + chiTietGioHangId + " đã được xóa trước đó.");
        }
        chiTietGioHang.setDeleted(true);
        chiTietGioHangRepository.save(chiTietGioHang);
        System.out.println("Marked ChiTietGioHang as deleted: " + chiTietGioHangId);

        ChiTietSanPham chiTietSanPham = chiTietGioHang.getIdChiTietSanPham();
        chiTietSanPham.setDeleted(false);
        chiTietSanPhamRepository.save(chiTietSanPham);
        System.out.println("Marked ChiTietSanPham as not deleted: " + chiTietSanPham.getId());

        List<HoaDonChiTiet> hoaDonChiTiets = hoaDonChiTietRepository.findByIdChiTietSanPhamIdAndDeletedFalse(chiTietSanPham.getId());
        for (HoaDonChiTiet hoaDonChiTiet : hoaDonChiTiets) {
            hoaDonChiTiet.setDeleted(true);
            hoaDonChiTietRepository.save(hoaDonChiTiet);
            System.out.println("Marked HoaDonChiTiet as deleted: " + hoaDonChiTiet.getId());
        }
        System.out.println("Successfully deleted ChiTietGioHang with ID: " + chiTietGioHangId);
    }
